...
class ResultControllerTest {
	...
	public void read() throws Exception {
		...
		ResultGetResponseDto resultDto = new ResultGetResponseDto(result);
	...
	@Test
	@DisplayName("결과 저장하기 테스트")
	public void getResultPostTest() throws Exception {

		ResultRequestDto resultRequestDto = new ResultRequestDto();
		resultRequestDto.setPatternId(1L);
		resultRequestDto.setAccessoryId(1L);
		resultRequestDto.setColorId(1L);
		resultRequestDto.setFontId(1L);
		resultRequestDto.setWheelId(1L);

		// ObjectMapper 객체를 사용해 RequestBody 부분 String으로 바꿔서 json 형태로
		String json = new ObjectMapper().writeValueAsString(resultRequestDto);

		mockMvc.perform(
						MockMvcRequestBuilders.post("http://localhost:8080/api/results")
								.contentType(MediaType.APPLICATION_JSON)
